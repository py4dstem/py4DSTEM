# Defines the DiffractionStack class, which stores a 3D stack of multiple
# 2D diffraction-shaped datasets
...
class DiffractionStack(ArrayStack):
    """
    Stores multiple diffraction-space shaped 2D data arrays.
    """
    def __init__(
        self,
        data: np.ndarray,
        name: Optional[str] = 'diffractionstack',
        ...
        Returns:
            A new DiffractionStack instance
        """
...
def DiffractionStack_from_h5(group:h5py.Group, name:str):
    """
    ...
    and if it does, loads and returns it as a DiffractionStack. If it doesn't exist, or if
    ...
    Returns:
        A DiffractionStack instance
    """
    diffractionstack = Array_from_h5(group, name)
    diffractionstack = DiffractionStack_from_Array(diffractionstack)
    return diffractionstack


def DiffractionStack_from_Array(array):
    """
    Converts an Array to a DiffractionStack.
    ...
    array.__class__ = DiffractionStack
    array.__init__(
        data = array.data,
        name = array.name,
