...
class Crystal_Phase:
    ...
    def __init__(
        ...
        if orientation_maps is None:
            self.orientation_maps = [
                crystals[ind].orientation_map for ind in range(self.num_crystals)
            ]
        else:
        ...
        if crystal_names is None:
            self.crystal_names = [
                "crystal" + str(ind) for ind in range(self.num_crystals)
            ]
        else:
        ...
        for a0 in range(self.num_crystals):
            ...
            for a1 in range(self.num_matches[a0]):
                self.crystal_identity = np.append(
                    self.crystal_identity,
                    np.array((a0, a1), dtype="int")[None, :],
                    axis=0,
                )
    ...
    def quantify_single_pattern(
        ...
        if power_intensity_experiment == 0:
            ...
            intensity = (
                bragg_peaks.data["intensity"][keep] ** power_intensity_experiment
            )
            intensity0 = (
        ...
        for a0 in range(self.num_fits):
                ...
                for a1 in range(strain_iterations):
                    ...
                    if np.sum(pair_sub) >= 4:
                        ...
                        bragg_peaks_fit.data["qx"] = (
                            qx_copy * m_strain[0, 0] + qy_copy * m_strain[1, 0]
                        )
                        bragg_peaks_fit.data["qy"] = (
                            qx_copy * m_strain[0, 1] + qy_copy * m_strain[1, 1]
                        )
            ...
            for a1 in range(bragg_peaks_fit.data.shape[0]):
                ...
                if include_false_positives:
                    weight = np.clip(
                        1 - np.sqrt(dist2[ind_min]) / corr_kernel_size, 0, 1
                    )
            ...
            if single_phase:
                ...
                for a0 in range(self.num_crystals):
                    ...
                    while search is True:
                        ...
                        if (
                            np.count_nonzero(phase_weights_cand > 0.0)
                            <= max_number_patterns
                        ):
                ...
                sub = np.logical_not(self.crystal_identity[:, 0] == ind_best_fit)
        ...
        if verbose:
            ...
            for a0 in range(self.num_fits):
                ...
                m = self.crystal_identity[a0, 1]
                line = "{:>12} {:>8}   {:<12}".format(
                    f"{phase_weights[a0]:.2f}", m, self.crystal_names[c]
                )
                if a0 == ind_max:
                    ...
                    print(line)
            print("----------------------------")
            line = "{:>12} {:>15}".format(f"{sum(phase_weights):.2f}", "fit total")
            print("\033[1m" + line + "\033[0m")
            line = "{:>12} {:>15}".format(f"{phase_residual:.2f}", "fit residual")
            print(line)
        ...
        if plot_result:
            ...
            if plot_correlation_radius:
                ax_leg.text(dy_leg, -dx_leg, "Correlation radius", **text_params)
                    ...
                    if plot_only_nonzero_phases is False or phase_weights[a0] > 0:
                        ...
                        if plot_unmatched_peaks:
                            ax.scatter(
                                ...
                                s=scale_markers_calculated
                                * int_fit[np.logical_not(matches_fit)],
                                marker=mvals[c],
                                facecolor=phase_colors[c, :],
                            )
    ...
    def plot_phase_weights(
        ...
        for a0 in range(self.num_crystals):
            ...
            im = np.clip(
                (im - weight_range[0]) / (weight_range[1] - weight_range[0]), 0, 1
            )
            ax[a0].imshow(
                im,
                vmin=0,
        ...
        # plot residuals
        im = np.clip(
            (phase_residuals - weight_range[0]) / (weight_range[1] - weight_range[0]),
            0,
            1,
        )
