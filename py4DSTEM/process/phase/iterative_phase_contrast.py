...
class PhaseReconstruction(metaclass=ABCMeta):
    ...
    def _extract_intensities_and_calibrations_from_datacube(
        ...
        if real_space_units == 'pixels':
            ...
            raise ValueError(f"Real-space calibrations must be given in 'A', not {real_space_units}")

        # Reciprocal-space
        if reciprocal_space_units == 'pixels':
            if require_calibrations:
                raise ValueError("Reciprocal-space calibrations must be given in in 'A^-1' or 'mrad'")
            ...
            if self._energy is not None:
                self._angular_sampling = (reciprocal_size*electron_wavelength_angstrom(self._energy)*1e3,)*2
            ...
            if self._energy is not None:
                self._reciprocal_sampling = (angular_size/electron_wavelength_angstrom(self._energy)/1e3,)*2
                self._reciprocal_units = ('A^-1',)*2
        else:
            raise ValueError(f"Reciprocal-space calibrations must be given in 'A^-1' or 'mrad', not {reciprocal_space_units}")
    ...
    def _calculate_intensities_center_of_mass(
        ...
        **kwargs):
        ...
        # calculate CoM
        self._com_measured_x = xp.sum(intensities*kxa[None,None],axis=(-2,-1))/self._intensities_sum
        self._com_measured_y = xp.sum(intensities*kya[None,None],axis=(-2,-1))/self._intensities_sum
        ...
        # fix CoM units
        self._com_normalized_x = (self._com_measured_x - self._com_fitted_x) * self._scan_sampling[0]
        self._com_normalized_y = (self._com_measured_y - self._com_fitted_y) * self._scan_sampling[1]
        ...
        if plot_center_of_mass:
            ...
            extent = [0,self._scan_sampling[0]*self._intensities_shape[0],0,self._scan_sampling[1]*self._intensities_shape[1]]
            fig,ax = plt.subplots(2,2,figsize=figsize)

            ax[0,0].imshow(asnumpy(self._com_measured_x),extent=extent,cmap=cmap,**kwargs)
            ax[0,1].imshow(asnumpy(self._com_measured_y),extent=extent,cmap=cmap,**kwargs)
            ax[1,0].imshow(asnumpy(self._com_normalized_x),extent=extent,cmap=cmap,**kwargs)
            ax[1,1].imshow(asnumpy(self._com_normalized_y),extent=extent,cmap=cmap,**kwargs)
    ...
    def _solve_for_center_of_mass_relative_rotation(
        ...
        **kwargs):
        ...
        # Untransposed
        com_measured_x = xp.cos(rotation_angles_rad) * self._com_normalized_x[None] - xp.sin(rotation_angles_rad) * self._com_normalized_y[None]
        com_measured_y = xp.sin(rotation_angles_rad) * self._com_normalized_x[None] + xp.cos(rotation_angles_rad) * self._com_normalized_y[None]
        ...
        if maximize_divergence:
            ...
            self._rotation_div = xp.mean(xp.abs(com_grad_x_x + com_grad_y_y),axis=(-2,-1))
        else:
            com_grad_x_y = com_measured_x[:,1:-1,2:] - com_measured_x[:,1:-1,:-2]
            com_grad_y_x = com_measured_y[:,2:,1:-1] - com_measured_y[:,:-2,1:-1]
            self._rotation_curl = xp.mean(xp.abs(com_grad_y_x - com_grad_x_y),axis=(-2,-1))

        # Transposed
        com_measured_x = xp.cos(rotation_angles_rad) * self._com_normalized_y[None] - xp.sin(rotation_angles_rad) * self._com_normalized_x[None]
        com_measured_y = xp.sin(rotation_angles_rad) * self._com_normalized_y[None] + xp.cos(rotation_angles_rad) * self._com_normalized_x[None]

        if maximize_divergence:
            com_grad_x_x = com_measured_x[:,2:,1:-1] - com_measured_x[:,:-2,1:-1]
            com_grad_y_y = com_measured_y[:,1:-1,2:] - com_measured_y[:,1:-1,:-2]
            self._rotation_div_transpose = xp.mean(xp.abs(com_grad_x_x + com_grad_y_y),axis=(-2,-1))
        else:
            com_grad_x_y = com_measured_x[:,1:-1,2:] - com_measured_x[:,1:-1,:-2]
            com_grad_y_x = com_measured_y[:,2:,1:-1] - com_measured_y[:,:-2,1:-1]
            self._rotation_curl_transpose = xp.mean(xp.abs(com_grad_y_x - com_grad_x_y),axis=(-2,-1))
        ...
        if maximize_divergence:
            ...
            if self._rotation_div[ind_max] >= self._rotation_div_transpose[ind_trans_max]:
            ...
            if self._rotation_curl[ind_min] <= self._rotation_curl_transpose[ind_trans_min]:
        ...
        if self._rotation_best_tranpose is False:
            self._com_x = xp.cos(self._rotation_best_rad) * self._com_normalized_x - xp.sin(self._rotation_best_rad) * self._com_normalized_y
            self._com_y = xp.sin(self._rotation_best_rad) * self._com_normalized_x + xp.cos(self._rotation_best_rad) * self._com_normalized_y
        else:
            self._com_x = xp.cos(self._rotation_best_rad) * self._com_normalized_y - xp.sin(self._rotation_best_rad) * self._com_normalized_x
            self._com_y = xp.sin(self._rotation_best_rad) * self._com_normalized_y + xp.cos(self._rotation_best_rad) * self._com_normalized_x
        ...
        if plot_rotation:
            ...
            ax.plot(
                rotation_angles_deg,
                asnumpy(self._rotation_div) if maximize_divergence else asnumpy(self._rotation_curl),
                label='CoM',
                )
            ax.plot(
                rotation_angles_deg,
                asnumpy(self._rotation_div_transpose) if maximize_divergence else asnumpy(self._rotation_curl_transpose),
            ...
            if maximize_divergence:
                ...
                ax.set_aspect(np.ptp(rotation_angles_deg)/np.ptp(self._rotation_div)/4)
            else:
                ax.set_ylabel('Mean Absolute Curl')
                ax.set_aspect(np.ptp(rotation_angles_deg)/np.ptp(self._rotation_curl)/4)
            fig.tight_layout()
            plt.show()
...
class DPCReconstruction(PhaseReconstruction):
    ...
    def _forward(
        ...
        obj_dx = 0.5 * (xp.roll(padded_phase_object,1,axis=0) - xp.roll(padded_phase_object,-1,axis=0))
        obj_dy = 0.5 * (xp.roll(padded_phase_object,1,axis=1) - xp.roll(padded_phase_object,-1,axis=1))
        ...
        error = xp.sqrt(xp.mean((obj_dx[mask] - xp.mean(obj_dx[mask]))**2 + (obj_dy[mask] - xp.mean(obj_dy[mask]))**2))
    ...
    def _adjoint(
        ...
        phase_update = xp.real(
                xp.fft.ifft2(
                    xp.fft.fft2(obj_dx)*kx_op + xp.fft.fft2(obj_dy)*ky_op
                )
            )
    ...
    def reconstruct(
        ...
        # initialization
        padded_object_shape = np.round(self._intensities_shape[:2] * padding_factor).astype('int')
        ...
        for a0 in tqdmnd(
            ...
            # update
            padded_phase_object = self._update(padded_phase_object,phase_update,step_size)

            if store_iterations:
                self._object_phase_iterations.append(padded_phase_object[:self._intensities_shape[0],:self._intensities_shape[1]].copy())
                self._error_iterations.append(error.item())

        # crop result
        self._object_phase = padded_phase_object[:self._intensities_shape[0],:self._intensities_shape[1]]
        ...
        **kwargs):
        ...
        for n,spec in enumerate(gridspec):
            ...
            if plot_convergence and n == len(grid_range):
                ...
                ax = fig.add_subplot(spec)
                im = ax.imshow(asnumpy(phases[grid_range[n]]),extent=extent,cmap=cmap,**kwargs)
                ax.set_xlabel(f"x [{self._scan_units[0]}]")
                ax.set_ylabel(f"y [{self._scan_units[1]}]")
                ax.set_title(f"Iteration: {grid_range[n]}\nRMS error: {errors[grid_range[n]]:.3f}")
