...
class PtychographicReconstruction(PhaseReconstruction):
    ...
    def _fourier_proximal_operator(
        self, amplitudes, fourier_arg_vector
        ):
        """
        """
        xp = self._xp

        return amplitudes * xp.exp(1j * xp.angle(fourier_arg_vector))
    ...
    def _shift_probes_and_patch_object(
        self,
        current_object,
        current_probe,
        ):
        ...
        shifted_probes = self._shift_probes(current_probe)
        object_patches = self._patch_object(current_object)
        return shifted_probes, object_patches

    def _overlap_proximal_operator(
        self,
        current_object,
        current_probe,
        fourier_arg_vector,
        ...
        xp = self._xp
        arg_vector = xp.fft.ifft2(fourier_arg_vector)

        shifted_probes, object_patches = self._shift_probes_and_patch_object(
                current_object,current_probe
                )
        
        fourier_overlap = xp.fft.fft2(shifted_probes * object_patches)
        error = (
        ...
        current_object = (
            self._sum_overlapping_patches_bincounts(
                xp.conj(shifted_probes) * arg_vector
            )
            * probe_normalization
        )
        ...
        if not fix_probe:
            ...
            current_probe = (
                xp.sum(
                    xp.conj(object_patches) * arg_vector,
                    axis=0,
                )
        ...
        fourier_arg_vector = xp.fft.fft2(shifted_probes * object_patches)
        
        return current_object, current_probe, fourier_arg_vector, error
    ...
    def reconstruct(
        ...
        shifted_probes, object_patches = self._shift_probes_and_patch_object(
                self._object, self._probe
                )
        self._fourier_vector_2 = xp.fft.fft2(shifted_probes * object_patches)
        self._fourier_auxiliary = xp.zeros_like(self._fourier_vector_2)
        ...
        for a0 in tqdmnd(
            ...
            self._fourier_vector_1 = self._fourier_proximal_operator(
                self._amplitudes,
                self._fourier_vector_2 - self._fourier_auxiliary
                )

            self._object, self._probe, self._fourier_vector_2, error = self._overlap_proximal_operator(
                self._object,
                self._probe,
                self._fourier_vector_1 + self._fourier_auxiliary,
                self._amplitudes,
                normalization_min = normalization_min,
                fix_probe=a0 < fix_probe_iter,
                )

            self._fourier_auxiliary += self._fourier_vector_1 - self._fourier_vector_2
