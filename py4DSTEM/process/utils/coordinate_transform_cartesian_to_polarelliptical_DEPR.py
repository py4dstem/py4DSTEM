...
class polar_elliptical_transform(object):
    ...
    where (x,y) is a point in the cartesian plane, (x0,y0,A,B,C) are parameters describing the
    transformation, and (xr,yr) are shorthand for

        xr = r*cos(theta)
        yr = r*sin(theta)

    Thus the transformation is from (x,y) sace to (r,theta) space.  For A=B, the (r,theta)
    parameterization describes a polar coordinate system.  For A!=B, the (r,theta)
    parameterization describes a polar elliptical coordinate system in which A and B
    are the semi-major axis lengths, and C corresponds to a tilt of the theta=0 direction
    relative to (x,y) space.

    The parameteris (x0,y0,A,B,C) are stored as elements of a length-5 array, in this order, at
    self.coeffs.  theta is passed in degrees (sorry and/or you're welcome).
    ...
    def get_polar_transform(self, ar=None, mask=None, return_ans=False):
        ...
        x = self.coefs[0] + self.coefs[2]*xr + self.coefs[4]*yr
        y = self.coefs[1] + self.coefs[3]*yr + self.coefs[4]*xr
