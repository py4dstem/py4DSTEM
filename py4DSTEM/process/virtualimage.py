...
def make_bragg_disk_mask(
    ...
    '''
    Function to make multiple circular masks at a fixed distance from a center,
    such as Bragg disks around a central beam. There are two methods to specify
    the distance from the center to make the masks. The first uses a 'bragg_distance'
    and 'offset_angle' to generate vectors for specifying disks. The second uses
    the vector between the center and one 'end_point' to specify mask locations.
    Only one method can be specified.

    Args:
        shape (tuple): defines shape of mask, for example (Q_Nx, Q_Ny) where Q_Nx
            and Q_Ny are the mask size
        radius (float): radius of circular masks in pixels
        num_disks(int): number of circular masks to make
        center (2-tuple): 'point' (qx,qy) to center masks around. If None, uses
            center of array
        bragg_distance (float): for first method, distance from center to
            circular masks in pixels
        offset_angle(float): for first method, angle to rotate the masks in
            radians, if offset_angle = 0, the first mask is along the positive
            x-axis.
        end_point (2-tuple): for second method, center of first circular disk.
            Used to generate other mask positions.
        return_sum (bool): If True, return all the circular masks summed together.
            If False, returns array of masks.
    ...
    assert any(a is not None for a in [bragg_distance, end_point]), "specify either 'bragg_distance' or 'end_point'"
    assert not all([bragg_distance, end_point]), "specify either 'bragg_distance' or 'end_point'"

    #create initial vector
    if bragg_distance:
        v0 = (bragg_distance,0)
    if end_point:
        v0 = (end_point[0]-center[0], end_point[1]-center[1])

    #calculate parameters for masks
    disk_centers = np.zeros((num_disks,2))
    ...
    for a0 in range(num_disks):
        masks[a0] = make_detector(
            shape,
            mode = 'circle',
            geometry = ((disk_centers[a0]), radius),
        )
