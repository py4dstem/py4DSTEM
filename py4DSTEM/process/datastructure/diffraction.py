...
class DiffractionSlice(DataObject):
    ...
    def __init__(self, data, parentDataCube, slicelabels=None, **kwargs):
        ...
        if len(shape)==2:
            assert shape==(self.Q_Ny,self.Q_Nx), "Shape of data is {}, but shape of diffraction space in parent DataCube is ({},{}).".format(shape, self.R_Ny, self.R_Nx)
            self.depth=1
            self.data2D = data
        else:
            assert shape[1:]==(self.Q_Ny,self.Q_Nx), "Shape of data is {}, but shape of diffraction space in parent DataCube is ({},{}).".format(shape, self.R_Ny, self.R_Nx)
            self.depth=shape[0]
