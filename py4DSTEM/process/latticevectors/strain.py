...
def get_rotated_strain_map(unrotated_strain_map, ux, uy):
    ...
    assert np.all([key in ['e_xx','e_xy','e_yy','theta'] for key in unrotated_strain_map.slices.keys()])
    ...
    rotated_strain_map = RealSlice(data=np.zeros((Rx,Ry,4)),
                                   ...
                                   name=unrotated_strain_map.name+"_rotated_by_{:.2f}".format(np.degrees(theta)))
